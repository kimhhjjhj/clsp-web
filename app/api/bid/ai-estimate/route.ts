...
import { estimateFromPreset } from '@/lib/cost-baseline'
...
export async function POST(req: NextRequest) {
  ...
  // mode=preset 또는 API 키 없음 → 룰 기반 (2026-04 단가표) 즉시 반환
  const url = new URL(req.url)
  const forcePreset = url.searchParams.get('mode') === 'preset'
  if (forcePreset || !process.env.ANTHROPIC_API_KEY) {
    const estimate = estimateFromPreset({
      type: body.type,
